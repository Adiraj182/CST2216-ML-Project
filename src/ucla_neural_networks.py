...
def run_admission_model(data_path='Datasets/Admission.csv', mlp_hidden_neurons=3, mlp_max_iter=200, mlp_learning_rate=0.001):
    ...
    try:
        ...
        results['scaler'] = scaler   # <-- returned so app.py can scale user input
        
        logger.info("Training MLPClassifier (ReLU)")
        MLP = MLPClassifier(
            hidden_layer_sizes=(mlp_hidden_neurons,),
            batch_size=50,
        ...
        results['relu_model'] = MLP   # <-- returned for live prediction
        ...
        results['tanh_model'] = MLP_tanh   # <-- returned for live prediction
